...
- 先完成 S02 前段交接，再按当前地图、天气、处理点、任务收益、阻挡和敌方设卡动态规划去 S14 的路线；当前地图在普通天气下通常会自然进入 S04/S05 水路。
- 在固定处理站自动提交 `PROCESS`。
...
- `BREAK_ORDER` 只在服务端公开 `breakOrderReady=true` 且冲刺急策额度可用时绑定到关键攻坚或宫门验核，不会单独发送。
...
- `auto`：默认值。完全按当前地图、天气、处理耗时、任务收益和阻挡做动态图搜索，不强制水路。
...
- `guard.log`：设卡/破关专用复盘，按中文分段记录车队位置、路线边朝向、关卡状态、关键判断和最终决策。
