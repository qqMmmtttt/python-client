...
| 文件 | 作用 |
|---|---|
| `planning/route_profiles.py` | 第一轮地图水路主路线，用于显式水路 profile 调试和专项策略。 |
...
- `auto`：默认值。交付策略先保证 S02 前段交接；之后完全按当前地图、天气、处理耗时、道路障碍、清障残留和敌方设卡做动态图搜索。
- `first-round-water`：强制使用第一轮水路主线。
- `first-round-safe`：兼容旧参数名，当前等价于 `first-round-water`。
- `generic`：完全按当前地图动态图搜索；当前实现与 `auto` 的路由语义一致，保留为配置兼容入口。
...
管线按任务书的动作类别上限合并动作：同帧最多 1 个主车队动作、1 个小分队动作、1 个窗口出牌动作和 1 个急策额度。多个策略给出同类别动作时按优先级裁决，高优先级动作会替换低优先级动作；不同类别会合并提交，例如主车队 `MOVE` 可以和 `SQUAD_SCOUT` 同帧发送。

| 策略 | 文件 | 职责 |
|---|---|---|
| 窗口策略 | `strategies/combat.py` | 参与窗口争夺时出牌，优先使用护卫点、献贡、文书、强行。 |
| 小分队策略 | `strategies/squad.py` | 在普通阶段提前探路 S04/S05/S11/S13/S14；沿当前 `RoutePolicy` 未来路径发现敌方设卡时优先削弱，发现道路障碍时派小分队清障。 |
| 任务策略 | `strategies/tasks.py` | 当前节点可安全完成任务时提交 `CLAIM_TASK`，并记忆被拒任务避免重复卡死。 |
| 资源策略 | `strategies/resources.py` | 领取冰鉴、马类、文书、情报和码头船权；按规则使用冰鉴、马类和 `INTEL`；路线边只允许马类资源。 |
| 急策策略 | `strategies/rush.py` | 冲刺阶段根据时间和鲜度选择 `RUSH_SPEED` 或 `RUSH_PROTECT`。 |
| 设卡策略 | `strategies/guard.py` | 达到 90 分任务基础线且交付余量充足时，在对手未来会经过的关键关隘/宫门等高价值节点保守设卡。 |
...
- 协议动作构造、消息构造、分帧。
- 地图、状态、天气解析。
- 策略管线类别合并、优先级替换与同类别冲突跳过。
- 任务规划、T04 障碍处理。
- S14 验核、S15 交付、S04 处理、固定处理重入。
- 路线边 `WAITING/MOVING` 资源限制回归。
- 冲刺阶段 `RUSH_SPEED`、`RUSH_PROTECT` 使用。
- 默认动态路线选择、暴雨回避水路、显式第一轮水路 profile。
- 情报领取/使用、小分队探路/清障、保守主动设卡。
- 日志分级输出。
...
| 测试系统 | 85% | 核心策略和回归用例已覆盖，缺完整比赛仿真器。 |
| 路线规划 | 87% | 默认 `auto` 支持天气/处理/阻挡/设卡代价的动态路线，显式 profile 保留第一轮水路；可继续加入更精细鲜度风险和对手预测代价。 |
| 任务策略 | 75% | 支持安全时间窗、拒绝记忆和 90/110/130 阈值排序；尚未做完整任务组合优化。 |
| 资源策略 | 86% | 支持冰鉴、马类、情报、文书和码头船权的主动领取/使用；仍缺对手库存推断。 |
| 对抗策略 | 78% | 有基础窗口出牌、小分队探路/清障/削弱、阻挡处理和按候选点评分的保守主动设卡；悬赏收益建模待扩展。 |
