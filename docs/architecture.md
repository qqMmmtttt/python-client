...
| 文件 | 作用 |
|---|---|
| `planning/route_profiles.py` | 第一轮地图保守主路线和地图签名。 |
...
- `auto`：默认值。第一轮地图签名匹配时使用保守主路线，否则回退动态图搜索。
- `first-round-safe`：强制使用第一轮保守路线。
- `generic`：完全按当前地图动态图搜索，便于决赛地图变化。
...
- S14：非冲刺阶段不验核，冲刺阶段提交 `VERIFY_GATE`；时间极晚且急策可用时绑定 `BREAK_ORDER`。
...
|---|---|
| `info.log` | 调试和普通运行信息。 |
| `important.log` | 开局、每帧状态、任务、天气、事件、动作结果和最终动作。 |
| `error.log` | 错误和异常堆栈。 |

`important.log` 是赛后复盘主文件，能够直接定位任务、天气、事件、拒绝码和动作决策。
...
| 数据模型 | 85% | 覆盖核心状态、地图、天气、玩家资源；小分队和悬赏可继续细化。 |
| 日志系统 | 90% | 已分级落盘，重要日志足够复盘当前策略。 |
| 测试系统 | 80% | 核心策略和回归用例已覆盖，缺完整比赛仿真器。 |
| 路线规划 | 80% | 支持第一轮固定路线和决赛动态图搜索；可继续加入更精细天气/对手代价。 |
| 任务策略 | 70% | 支持安全时间窗和拒绝记忆；尚未做复杂对手博弈。 |
| 资源策略 | 75% | 支持冰鉴、马类、关键资源领取；情报、文书资源主动规划待扩展。 |
| 对抗策略 | 55% | 有基础窗口出牌和阻挡处理；小分队、设卡、防守收益待扩展。 |
| 交付策略 | 85% | 当前地图可完整走到 S15 交付；极端对手干扰仍需更强仿真和博弈。 |
